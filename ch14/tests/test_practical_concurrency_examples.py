...
class TestAsyncBatchProcessor:
    ...
    async def test_process_batch_returns_all_items(self):
        """Test that process_batch returns all items."""
        processor = AsyncBatchProcessor()
        batch = [1, 2, 3, 4, 5]
    ...
    @pytest.mark.asyncio
    async def test_process_all_returns_all_items(self):
        """Test that process_all returns all items."""
    ...
    async def test_process_all_correct_batching(self):
        ...
        # 10 items with batch_size 3 = 4 batches (3+3+3+1)
        assert len(results) == 10
...
class TestEdgeCases:
    ...
    def test_dataprocessor_zero_value(self):
        """Test data processor with zero value."""
        record = DataRecord(1, 0.0, "test")
        processed = DataProcessor.process_record(record)

        assert processed.value == 0.0

    def test_dataprocessor_negative_value(self):
        """Test data processor with negative value."""
        record = DataRecord(1, -5.0, "test")
        processed = DataProcessor.process_record(record)
